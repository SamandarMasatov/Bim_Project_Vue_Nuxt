          ...
          <v-row justify="center">
            ...
            <v-col cols="4">
              <div style="position: relative">
                ...
                <div class="datass" :class="t ? 'datass-active' : ''">
                  <v-btn @click="t = false" class="datas-close" elevation="0">
                    <v-icon>mdi-close</v-icon>
                  </v-btn>
                  <div
                    class="no-data"
                    :class="number.length == 0 ? '' : 'd-none'"
                  >
                    {{ $t('noResult') }}
                  </div>
                  <div class="data" v-for="(num, i) in number" :key="i">
                    <NuxtLink
                      :to="`/${$i18n.locale}/Constraction/${num._id}`"
                      class="link my-3"
                    >
                      <p class="model-name mb-2">
                        {{ num.name[$i18n.locale] }}
                      </p>
                    </NuxtLink>
                  </div>
                </div>
          ...
          <div class="project" id="project">
            <div class="project_in">
              ...
              >
                  <div
                    cols="7"
                    class="project_card_img pa-0"
                  >
                    <v-carousel hide-delimiters height="100%">
                      <v-carousel-item
                        :to="`/${$i18n.locale}/Constraction/${model._id}`"
                        v-for="(item, i) in model.file"
                        :key="i"
                        :src="`http://api.bimproject.uz/uploads/cons/${item}`"
                      ></v-carousel-item>
                    </v-carousel>
                  </div>
                  <div cols="5" class="project_card_text d-flex flex-column justify-space-between py-3">
                    <div class="title">{{ model.name[$i18n.locale] }}</div>
                    <div class="datas">
                      <div>
                        <span>{{ $t('razmer') }}</span> <span>{{ model.size }}</span>
                      </div>
                      <div>
                        <span>{{ $t('faylTuri') }}:</span>
          ...
          for (let a = 0; a < this.models2.length; a++) {
            for (let b = 0; b < this.items[i].items.length; b++) {
              if (
                ...
                this.models2[a].ConsType_ID == this.items[i].items[b].id
              ) {
